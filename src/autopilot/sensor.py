...
class Sensor:
    ...
    def __init__(self, dim, constant_noise, randseed):
        ...
        match np.ndim(constant_noise):
            case 0:
                self.cov_matrix = np.eye(dim) * constant_noise**2
            case 1:
                self.cov_matrix = np.diag(constant_noise**2)
            case 2:
                self.cov_matrix = constant_noise**2
            case _:
                assert False, (
                    'Unexpected number of dimensions in parameter `constant_noise`'
                )
...
class IntermittentSensor(Sensor):
    ...
    def is_available(self, true_state, time):
        ...
        if time - self.last_sense_time > self.sensing_interval:
            self.last_sense_time = (
                time // self.sensing_interval
            ) * self.sensing_interval
            return True
        else:
            return False
...
class DistanceSignal(IntermittentSensor):
    ...
    def new_reading(self, true_state):
        distance = np.linalg.vector_norm(
            true_state[sd.POS] - self.satellite_state[sd.POS]
        )
        assert self.cov_matrix.size == 1
        self.reading = (
            distance + self.rng.normal(0, np.sqrt(self.cov_matrix))
        ).reshape(1)
        return self.reading
...
class SensorSystem_EKF(SensorSystem):
    ...
    def sense(self, true_state, timestamp):
        ...
        active_sensors = [
            s for s in self.sensors if s.is_available(true_state, timestamp)
        ]
        H = np.vstack(
            tuple(
                np.atleast_1d(s.linearized_model(self.EKF.xp)) for s in active_sensors
            )
        )
        zp = np.concat(
            tuple(np.atleast_1d(s.predict_reading(self.EKF.xp)) for s in active_sensors)
        ).T
        z = np.concat(
            tuple(np.atleast_1d(s.new_reading(true_state)) for s in active_sensors)
        ).T
