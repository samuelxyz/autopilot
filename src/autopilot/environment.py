...
atmo_levels = np.array((-2000.63, 0, 11e3, 20e3, 32e3, 47e3, 51e3, 71e3, 84852, 88744)) # meters
pressure_refs = np.array((108900, 101325, 22632.1, 5474.89, 868.019, 110.9063, 66.9389, 3.95642, 0.373384, 0.183360)) # Pa
temp_refs = np.array((292.05, 288.15, 216.65, 216.65, 228.65, 270.65, 270.65, 214.65, 186.946, 186.946)) # K
temp_grads = np.array((-1.95, -6.5, 0, 1, 2.8, 0, -2.8, -2, 0, (730-187)/(150-90)))*1e-3 # K/m
...
def earth_atmo_pressure(altitude):
    ...
    if (L := temp_grads[i]) != 0.:
        ...
        return pressure_refs[i] * np.exp(-g0*M0*(H-atmo_levels[i])/(Rgas*temp_refs[i]))
...
class Earth(Body):
    ...
    def get_normal_shock_temperature(self, ship_state):
        mach = self.get_mach_number(ship_state)
        ratio = ((1 + (gamma-1)/2 * mach**2) * (2*gamma/(gamma-1) * mach**2 - 1)
...
def make_environment_ECI():
    ...
    '''
    return Environment([
        Earth(sd.make_state(angvel=(0, 0, 2*np.pi/86164.0905)))
    ])
