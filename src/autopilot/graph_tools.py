...
class Grapher:
    ...
    def plot_est_component(self, ax: Axes, component_index, label, color=None):
        ...
        ax.fill_between(
            ...
            line - stdev,
            line + stdev,
            alpha=self.uncertainty_transparency,
            color=color,
        )
        ax.set_xlabel(self.time_label)
        ax.grid(True)

    def plot_component_errs(
        self, ax: Axes, component_index, vec_label, component_names=None
    ):
        ...
        """
        err_components = (
            self.est_state[:, component_index] - self.true_state[:, component_index]
        )
        std_components = np.sqrt(
            np.diagonal(
                self.est_state_cov[:, component_index, component_index],
                axis1=-2,
                axis2=-1,
            )
        )
    ...
    def plot_orientation_err(self, ax: Axes, color=None):
        ...
        if self.est_state_cov is not None:
            ori_component_sd = np.sqrt(
                np.diagonal(
                    self.est_state_cov[:, sd.ORIENT, sd.ORIENT], axis1=-2, axis2=-1
                )
            )
            ori_rss_z = np.linalg.norm(
                quat.as_float_array(ori_true - ori_est) / ori_component_sd, axis=-1
            )
            ...
            ax.fill_between(
                ...
                0,
                ori_stdev,
                label='Est Orientation Uncertainty',
                color=color,
                alpha=self.uncertainty_transparency,
            )
        ax.grid(True)
