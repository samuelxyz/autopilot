...
class Thrusters_Floating_6(Thruster):
    ...
    def __init__(self, max_accel=np.inf, min_accel=0.):
        super().__init__()
    ...
    def command(self, accel_xyz):
        ...
        clipped = np.clip(np.abs(accel_xyz), self.min_accel, self.max_accel) * np.sign(accel_xyz)
...
class Thrusters_Fixed_Binary_6(Thruster):
    ...
    def get_accel(self, state, mass, local_inertia_matrix):
        result = np.zeros(sd.QDOT_N)
        result[sd.LIN] = quat.rotate_vectors(sd.get_orient(state), self.thrusts)
        return result