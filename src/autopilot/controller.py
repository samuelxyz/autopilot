...
class Controller:
    ...
    def update(self, ship_state, mass, inertia_matrix_body, dt):
        pass
...
class Binary_Controller_3(Controller):
    ...
    def __init__(
        self, setpoint, thrust_spec, thruster: thruster.Thrusters_Fixed_Binary_6
    ):
        """thrust_spec: the thrust produced by firing one thruster"""
        super().__init__(thruster)
        self.setpoint = setpoint
    ...
    def update(self, ship_state, mass, inertia_matrix_body, dt):
        ...
        for axis in range(3):
            ...
            # v^2 - v_0^2 = 2ad
            stopping_distance = my_vel**2 / (2 * accel)
            if abs(target_pos) > stopping_distance * self.margin:
                # what happens if we speed up during this timestep?
                new_speed = np.abs(my_vel) + accel * dt
                new_stopping_distance = new_speed**2 / (2 * accel)
...
class Attitude_Controller_12(Controller):
    """Translational controller using body-fixed thrusters, two on each face of a cubic spacecraft.
    For now it assumes these thrusters will only be used in force-couple pairs for pure rotation control.
    """

    def __init__(self, setpoint, moment, thruster):
        """Parameters:
        setpoint: state vector, world frame components. linear pos/vel will be ignored
        moment: angular moment magnitude generated by a thruster couple when firing
        """
        super().__init__(thruster)
        self.setpoint = setpoint
        self.moment = moment
    ...
    def update(self, ship_state, mass, inertia_matrix_body, dt):
        excess_angvel_world = ship_state[sd.ANGVEL] - self.setpoint[sd.ANGVEL]
        into_body_frame_q = sd.get_orient_q(ship_state).conj()
        excess_angvel_body = sd.rotate_vector(into_body_frame_q, excess_angvel_world)
        excess_orient_q_body = into_body_frame_q * sd.get_orient_q(self.setpoint).conj()
        excess_orient_vec_body = quat.as_rotation_vector(excess_orient_q_body)

        for axis in range(3):
            vel = excess_angvel_body[sd.ANGVEL + axis]
            pos = excess_orient_vec_body[sd.ANGVEL + axis]
